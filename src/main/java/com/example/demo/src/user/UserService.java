...
//  *********************************************************************
    ...
    // 회원정보 수정(Patch)
    public void modifyUserName(PatchUserReq patchUserReq, int idx) throws BaseException {
        try {
            int result = userDao.modifyUserName(patchUserReq, idx); // 해당 과정이 무사히 수행되면 True(1), 그렇지 않으면 False(0)입니다.
    ...
    public void deleteUser(int idx) throws BaseException{
        try {
            int result = userDao.deleteUser(idx);
            if (result == 0){
                throw new BaseException(MODIFY_FAIL_USERNAME);
            }
