...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into user (nickName, password, phoneNum, email) VALUES (?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getNickName(), postUserReq.getPassWord(), postUserReq.getPhoneNum(),postUserReq.getEmail()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    // 회원정보 변경
    public int modifyUserName(PatchUserReq patchUserReq, int idx) {
        String modifyUserNameQuery = "update user set nickname = ?, phoneNum = ?, email = ? where idx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchUserReq.getNickname(), patchUserReq.getPhoneNum(),patchUserReq.getEmail(), idx}; // 주입될 값들(nickname, userIdx) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select idx,nickname,password,phoneNum,email,status from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public int changeStatus(PostLoginReq postLoginReq){
        String changeStatusQuery = "update User set status = activate where email = ?";
        String email = postLoginReq.getEmail();
        return this.jdbcTemplate.update(changeStatusQuery, email);
    }

    // User 테이블에 존재하는 전체 유저들의 정보 조회
    public List<GetUserRes> getUsers() {
        String getUsersQuery = "select idx,nickname,password,phoneNum,email,status from user"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
    ...
    public List<GetUserRes> getUsersByNickname(String nickname) {
        ...
        return this.jdbcTemplate.query(getUsersByNicknameQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("password"),
                        rs.getString("phoneNum"),
                        rs.getString("email"), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
                        rs.getString("status"))                        // 해당 닉네임을 갖는 모든 User 정보를 얻기 위해 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
        );
    }

    // 해당 userIdx를 갖는 유저조회
    public GetUserRes getUser(int idx) {
        String getUserQuery = "select idx,nickname,password,phoneNum,email,status from User where idx = ?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
        ...
        return this.jdbcTemplate.queryForObject(getUserQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("email"), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
                        rs.getString("status")),
                getUserParams                  // 해당 닉네임을 갖는 모든 User 정보를 얻기 위해 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
        ); // 한 개의 회원정보를 얻기 위한 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }

    public int deleteUser(int idx) {
        String deleteUserQuery = "delete from user where idx = ?";
        return this.jdbcTemplate.update(deleteUserQuery,idx);
    }
}
